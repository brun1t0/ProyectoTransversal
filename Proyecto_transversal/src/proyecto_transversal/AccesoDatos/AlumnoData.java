...
public class AlumnoData {
    ...
    try {
        ...
        if (rs.next()) {
            ...
            JOptionPane.showMessageDialog(null, "Alumno añadido con éxito. ID:" + rs.getInt(1));
        }
        ps.close();
    ...
    public void modificarAlumno (Alumno alumno){
        String sql ="UPDATE alumno SET dni = ?, apellido = ?, nombre = ?, fechaNacimiento = ?, estado = 1 WHERE IdAlumno =?";
        PreparedStatement ps = null;
        try{
            ps = con.prepareStatement(sql);
            ps.setInt(1,alumno.getDni());
    ...
    public int obtenerIdAlumnoPorDNI(int dni) {
        int idAlumno = -1;
